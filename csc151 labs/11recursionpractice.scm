...
;; Authors: billy cherres Gabriela Roznawska
;; Date: october 5th
;; Acknowledgements:
...
; two of these problems. However, I highly, highly recommend you
...
; lst empty: return 1
; lst has head and tail: multiply the head to the tail to get the product of the list

; (product lst)-> integer?
; lst-> list? list of integers
; returns the product of the integers of the list
...
(test-case "null" equal? 1 (product (list)))
(test-case "basic test" equal? 12 (product (list 3 4)))
(test-case "three components" equal? 125 (product (list 5 5 5)))
(test-case "negative" equal? (- 12) (product (list (- 3) 4)))
; ------------------
"Problem 2: All"
; ------------------
...
; list is empty return #t
; list is non empty-> return #\t if all true and #\f if there is any that is false
; if the head is true then check the tail
; once it hits false then return false
...
(test-case "null" equal? #t (all (list)))
(test-case "simple true" equal? #t (all (list #t #t)))
(test-case "true true false" equal? #f (all (list #t #t #f)))
(test-case "simple false" equal? #f (all (list #f)))
...
; `(reverse l)` that returns list l but with the order of its
