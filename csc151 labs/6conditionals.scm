...
; `foo` and `bar`?
;
; bar has a more straight forward execution trace, foo has to go throughb the parameters first
; +---------------------------+--------------------------------------
...
; c. Why did we choose those three example inputs (that is, "2 3 4",
  ; "3 2 4" and "2 3 1")?
; 
; you guys chose this because they are all different types of scenerios in which the functions could be played out

; d. What do you see as the similarities and differences between
; `baz` and `qux`?
;
; similarities and that they both gave the same output for the most part and the difference was that baz was a quicker and less steps to execute
...
(define is-even-integer?
  (lambda (n)
    (cond
      [(and (integer? n )(even? n) ) "true"]
      [(and (integer? n )(odd? n) ) "false"]
      [else "false"])
    ))

(is-even-integer? "sdfdv")


; c. Write a function (number-not-integer? n) that returns true if `n`
; is a number that is not an integer and false otherwise.

(define number-not-integer? 
  (lambda (n) 
    (if (not (integer? n ))
      "true"
      "false")))

(number-not-integer? #\f)
...
(define better-both-even?
  (lambda (n1 n2)
    (if (and (even? n1) (even? n2))
      "both even" "both are not even")))

(better-both-even? 2 4)
...
;
; Predicates automatically produce #t and #f as outputs.. so there is no need
; to write it in the function. Take away the if statement and the boolean values and just keep the predicate and youll get the same output
...
(define founder-id? 
  (lambda (id)
    (if 
      ((and (= 6 (string-length id)) (= 000 (string->number(substring id 0 3 ))))) "valid founder id" "invalid founder id")))
...
(define yes?
  (lambda (str)
    (equal? (string->list str) (list #\y #\e #\s) )))
...
(define yes?
  (lambda (str)
    (cond
      [(equal? (string->list str) (list #\y #\e #\s))#t]
      [(equal? (string->list str) (list #\Y #\e #\s))#t]
      [(equal? (string->list str) (list #\y))#t]
      [else #f]
      )))
...
(define gpa->letter
  (lambda (g)
    ...
    [(and (>= g 3.5) (<= g 4.0))"A"]
    [(< g .5)"F"]
    [(> g 4.0) "invalid gpa entry"]
    [else "invalid gpa entry"])))
...
; a. Ensure that your combined file runs properly and it is named
; conditionals.scm
; b. Send this completed file to your partner for their records.
