...
(test-case "rate->samples-per-clip example 1"
  = (rate->samples-per-clip high-quality 1) 44100)
...
;;; (waves-per-clip sample-rate frequency duration) -> number?
;;;   sample-rate: number?, a non-negative integer
;;;   frequency: number?, a non-negative number 
;;;   duration: number?, a non-negative number
...
;    (append (vector 0 (/ (samples-per-wave sample-rate duration) 2) frequency)
;      (vector (/ (samples-per-wave sample-rate duration) 2)) (samples-per-wave sample-rate duration) frequency)))
...
"square"
(sample-node (square-sample medium-quality middle-c 2))

(square-sample 10 middle-c 2)
;; ------------------------
...
"sawtooth"
(sample-node (sawtooth-sample medium-quality middle-c 2))
...
"triangle"
(sample-node (triangle-sample medium-quality middle-c 2))
...
"sine"
(sample-node (sine-sample medium-quality middle-c 2))