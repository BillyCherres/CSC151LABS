...
(lambda (n x)
(if (equal? (+ 1 (random n) ) x) "you win" "you lose!"
...
(define loop-8
  (seq
    (note 51 (dur (random 80) 1))  ; 2 secs
...
(define eno
 ...
 (repeat n (par loop-1 loop-2 loop-3 loop-4 loop-5 loop-6 loop-7 loop-8)))
 )
...
; we changed all of the durations to be random so they play at random times

(mod (tempo qn 240) (eno 2))
